...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = PandaCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = PandaCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = PandaCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = PandaCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = PandaCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__panda__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: PandaLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for panda:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case PandaLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case PandaCanvasCommand.present.rawValue,
            ...
            case PandaCanvasA2UICommand.reset.rawValue,
            ...
            case PandaCameraCommand.list.rawValue,
                 PandaCameraCommand.snap.rawValue,
                 PandaCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case PandaScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: PandaNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: PandaNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(PandaLocationGetParams.self, from: req.paramsJSON)) ??
            PandaLocationGetParams()
        ...
        let payload = PandaLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case PandaCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(PandaCanvasPresentParams.self, from: req.paramsJSON)) ??
                PandaCanvasPresentParams()
        ...
        case PandaCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case PandaCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(PandaCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case PandaCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(PandaCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PandaCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(PandaCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case PandaCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.pandaA2UI) return JSON.stringify({ ok: false, error: "missing pandaA2UI" });
              return JSON.stringify(globalThis.pandaA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case PandaCanvasA2UICommand.push.rawValue, PandaCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == PandaCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(PandaCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try PandaCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(PandaCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try PandaCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.pandaA2UI) return JSON.stringify({ ok: false, error: "missing pandaA2UI" });
                const messages = \(messagesJSON);
                return JSON.stringify(globalThis.pandaA2UI.applyMessages(messages));
              } catch (e) {
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case PandaCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PandaCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(PandaCameraSnapParams.self, from: req.paramsJSON)) ??
                PandaCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case PandaCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(PandaCameraClipParams.self, from: req.paramsJSON)) ??
                PandaCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(PandaScreenRecordParams.self, from: req.paramsJSON)) ??
            PandaScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> PandaLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return PandaLocationMode(rawValue: raw) ?? .off
    }
