...
struct IOSGatewayChatTransport: PandaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PandaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(PandaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> PandaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(PandaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [PandaChatAttachmentPayload]) async throws -> PandaChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [PandaChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(PandaChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(PandaGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PandaGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PandaChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: PandaAgentEventPayload.self)
                        {
