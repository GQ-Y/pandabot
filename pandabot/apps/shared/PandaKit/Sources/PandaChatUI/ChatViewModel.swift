...
private let chatUILogger = Logger(subsystem: "bot.molt", category: "PandaChatUI")

@MainActor
@Observable
public final class PandaChatViewModel {
    public private(set) var messages: [PandaChatMessage] = []
    ...
    public var attachments: [PandaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [PandaChatPendingToolCall] = []
    public private(set) var sessions: [PandaChatSessionEntry] = []
    private let transport: any PandaChatTransport
    ...
    private var pendingToolCallsById: [String: PandaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any PandaChatTransport) {
    ...
    public var sessionChoices: [PandaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [PandaChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [PandaChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: PandaPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [PandaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: PandaChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [PandaChatMessage]) -> [PandaChatMessage] {
        var result: [PandaChatMessage] = []
    ...
    private static func dedupeKey(for message: PandaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [PandaChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> PandaChatAttachmentPayload in
            PandaChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> PandaChatSessionEntry {
        PandaChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: PandaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: PandaChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: PandaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = PandaChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> PandaPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
