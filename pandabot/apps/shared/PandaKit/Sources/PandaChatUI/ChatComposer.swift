...
@MainActor
struct PandaChatComposer: View {
    @Bindable var viewModel: PandaChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(PandaChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(PandaChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \PandaPendingAttachment.id)
                { (att: PandaPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            PandaPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(PandaChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(PandaChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(PandaChatTheme.subtleCard)
        .clipShape(Capsule())
    }
