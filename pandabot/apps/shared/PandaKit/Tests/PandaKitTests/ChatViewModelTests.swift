...
private final class TestChatTransport: @unchecked Sendable, PandaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [PandaChatHistoryPayload]
    private let sessionsResponses: [PandaChatSessionsListResponse]

    private let stream: AsyncStream<PandaChatTransportEvent>
    private let continuation: AsyncStream<PandaChatTransportEvent>.Continuation

    init(
        historyResponses: [PandaChatHistoryPayload],
        sessionsResponses: [PandaChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<PandaChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> PandaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? PandaChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [PandaChatAttachmentPayload]) async throws -> PandaChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return PandaChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> PandaChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? PandaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: PandaChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = PandaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = PandaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { PandaChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = PandaChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = PandaChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = PandaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { PandaChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
