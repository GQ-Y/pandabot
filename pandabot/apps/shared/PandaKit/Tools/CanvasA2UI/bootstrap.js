...
import "@a2ui/lit/ui";
import { themeContext } from "@pandabot/a2ui-theme-context";
...
class PandaA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--panda-a2ui-inset-top, 0px)
        var(--panda-a2ui-inset-right, 0px)
        var(--panda-a2ui-inset-bottom, 0px)
        var(--panda-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--panda-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--panda-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--panda-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--panda-a2ui-empty-top, var(--panda-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.pandaA2UI = api;
    globalThis.pandaA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    globalThis.addEventListener("panda:a2ui-action-status", this.#statusListener);
    this.#syncSurfaces();
  }
  ...
  disconnectedCallback() {
    ...
    if (this.#statusListener) {
      globalThis.removeEventListener("panda:a2ui-action-status", this.#statusListener);
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__pandaLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.pandaCanvasA2UIAction ??
      globalThis.webkit?.messageHandlers?.pandaCanvasA2UIAction ??
      globalThis.pandaCanvasA2UIAction ??
      globalThis.pandaCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (
          handler === globalThis.pandaCanvasA2UIAction ||
          handler === globalThis.pandaCanvasA2UIAction
        ) {
...
customElements.define("panda-a2ui-host", PandaA2UIHost);
