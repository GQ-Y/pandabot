...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: PandaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: PandaProtocol.AnyCodable] = [
            "presence": .init([PandaProtocol.AnyCodable(entry)]),
        ]
        let payload = PandaProtocol.AnyCodable(payloadMap)
