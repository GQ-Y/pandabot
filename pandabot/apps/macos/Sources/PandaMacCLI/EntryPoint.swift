...
struct PandaMacCLI {
    static func main() async {
        ...
        default:
            fputs("panda-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      ...
      panda-mac discover [--timeout <ms>] [--json] [--include-local]
      panda-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      panda-mac connect
      panda-mac connect --url ws://127.0.0.1:18789 --json
      panda-mac discover --timeout 3000 --json
      panda-mac wizard --mode local
    """)
}
