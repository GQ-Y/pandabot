...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "bot.panda", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in PandaBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: PandaBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "bot.panda.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        let domain = PandaBonjour.wideAreaGatewayServiceDomain
    ...
    private func updateGateways(for domain: String) {
        ...
        if domain == PandaBonjour.wideAreaGatewayServiceDomain,
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (Panda)", with: "")
