...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("panda").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing panda CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed panda \($0)." } ?? "Installed panda."
