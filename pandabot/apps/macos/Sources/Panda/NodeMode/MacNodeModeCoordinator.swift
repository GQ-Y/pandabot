...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "bot.panda", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: PandaNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [PandaCapability.canvas.rawValue, PandaCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(PandaCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if PandaLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(PandaCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(PandaCapability.camera.rawValue) {
        ...
        if capsSet.contains(PandaCapability.location.rawValue) {
