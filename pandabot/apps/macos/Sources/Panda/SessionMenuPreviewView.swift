...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "bot.panda", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> PandaSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: PandaSessionPreviewEntry,
    ...
    private static func cache(payload: PandaSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: PandaChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [PandaKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [PandaKit.AnyCodable]) -> [PandaChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(PandaChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: PandaChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: PandaChatMessage) -> Bool {
    ...
    private static func toolNames(for message: PandaChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: PandaChatMessage) -> String? {
