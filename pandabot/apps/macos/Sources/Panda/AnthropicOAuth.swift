...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Panda token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: PandaOAuthStore.AnthropicOAuthStatus = PandaOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "bot.panda", category: "anthropic-oauth")
...
enum PandaOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let PANDA_OAUTH_DIR = "PANDA_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Panda OAuth token file not found"
            case .unreadableFile: "Panda OAuth token file not readable"
            case .invalidJSON: "Panda OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Panda OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Panda OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.PANDA_OAUTH_DIR]?
