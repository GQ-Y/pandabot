...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: PandaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "bot.panda", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: PandaKit.AnyCodable]()) {
                $0[$1.key] = PandaKit.AnyCodable($1.value.base)
            }
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or PANDA_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: PandaProtocol.AnyCodable?) -> [String: PandaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: PandaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: PandaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: PandaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("panda.control.heartbeat")
    static let controlAgentEvent = Notification.Name("panda.control.agent")
}
