...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.panda/panda.json. Prefer PANDA_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.panda/panda.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.panda/panda.json. Restart the gateway to apply."
        }
