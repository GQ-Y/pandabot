...
enum PandaLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = PandaOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = PandaFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        PandaLogging.bootstrapIfNeeded()
        let label = PandaLogging.makeLabel(subsystem: subsystem, category: category)
...
struct PandaOSLogHandler: LogHandler {
...
struct PandaFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = PandaLogging.parseLabel(self.label)
