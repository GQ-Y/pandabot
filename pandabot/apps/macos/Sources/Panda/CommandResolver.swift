...
enum CommandResolver {
    private static let projectRootDefaultsKey = "panda.gatewayProjectRootPath"
    private static let helperName = "panda"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/panda.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/panda")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let pandaPaths = self.pandaManagedPaths(home: home)
        if !pandaPaths.isEmpty {
            extras.insert(contentsOf: pandaPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + pandaPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func pandaManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".panda")
    ...
    static func pandaExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectPandaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/panda.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyPandaInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.pandaExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let pandaPath = self.projectPandaExecutable(projectRoot: root) {
                return [pandaPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "panda", subcommand] + extraArgs
            }
            if let pandaPath = self.pandaExecutable(searchPaths: searchPaths) {
                return [pandaPath, subcommand] + extraArgs
            }

            let missingEntry = """
            panda entrypoint missing (looked for dist/index.js or bin/panda.js); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real panda CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/panda"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/panda.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/panda.js"
            node "$PRJ/bin/panda.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent panda"
          pnpm --silent panda \(quotedArgs);
        else
          echo "panda CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? PandaConfigFile.loadDict()
