...
private let webChatSwiftLogger = Logger(subsystem: "bot.panda", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: PandaChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> PandaChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(PandaChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> PandaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                PandaGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<PandaChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any PandaChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = PandaChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: PandaChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<PandaChatView>) -> NSViewController
    {
