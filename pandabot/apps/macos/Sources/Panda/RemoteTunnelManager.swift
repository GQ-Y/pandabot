...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "bot.panda", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Panda run already has an SSH listener on the expected port (common after restarts),
