...
enum DebugActions {
    private static let verboseDefaultsKey = "panda.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "panda.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Panda", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".panda/panda.json")
        guard
