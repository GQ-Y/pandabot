...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["PANDA_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/panda")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("panda-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("panda-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/panda/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("panda") && $0.pathExtension == "log" }
            .max { lhs, rhs in
