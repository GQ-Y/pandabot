...
This document describes how the Panda ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`panda acp` exposes an ACP agent over stdio and forwards prompts to a running
Panda Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Panda Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `panda acp` over stdio.

Example config:

```bash
panda config set gateway.remote.url wss://gateway-host:18789
panda config set gateway.remote.token <token>
```

Example run:

```bash
panda acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
panda acp --session agent:main:main
panda acp --session agent:design:main
panda acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Panda ACP” to start a thread.

## Execution Model

- ACP client spawns `panda acp` and speaks ACP messages over stdio.
...
```bash
panda acp --session agent:main:main
panda acp --session-label "support inbox"
panda acp --reset-session
```
...
`panda acp` resolves the Gateway URL and auth from CLI flags or config:
