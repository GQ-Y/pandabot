...
Delivery:
- Publish as `@pandabot/plugin-sdk` (or export from core under `pandabot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `PandaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: PandaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: PandaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: PandaConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: PandaConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: PandaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: PandaConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@pandabot/plugin-sdk`.
- Add `api.runtime` to `PandaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `pandaRuntime: ">=2026.2.0"`).
