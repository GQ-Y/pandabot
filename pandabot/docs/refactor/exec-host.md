...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.pandabot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.pandabot/node.json` (node id + pairing token).
...
Path: `~/.pandabot/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.pandabot/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.pandabot/exec-approvals.sock` (0600).
