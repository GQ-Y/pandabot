...
```bash
panda plugins install @panda/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
panda plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Panda" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `panda pairing list nextcloud-talk`
  - `panda pairing approve nextcloud-talk <CODE>`
