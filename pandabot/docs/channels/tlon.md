...
Tlon is a decentralized messenger built on Urbit. Panda connects to your Urbit ship and can
...
```bash
panda plugins install @panda/tlon
```

Local checkout (when running from a git repo):

```bash
panda plugins install ./extensions/tlon
```
...
Use these with `panda message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Panda replies in-thread.
