...
```bash
panda plugins install @panda/msteams
```

Local checkout (when running from a git repo):
```bash
panda plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Panda will offer the local install path automatically.
...
3) Configure Panda with those credentials.
...
## Goals
- Talk to Panda via Teams DMs, group chats, or channels.
...
- On startup, Panda resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.pandabot/panda.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Panda, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `panda-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `panda plugins install @panda/msteams`
   - From a local checkout: `panda plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Panda in Teams", "full": "Panda in Teams" },
...
Panda handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Panda only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/PandaShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
Panda sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `panda message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.pandabot/msteams-polls.json`.
...
**CLI:**
```bash
panda message send --channel msteams \
...
panda message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
panda message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
panda message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
panda message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
