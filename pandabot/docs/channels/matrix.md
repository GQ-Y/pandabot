...
Matrix is an open, decentralized messaging protocol. Panda connects as a Matrix **user**
...
```bash
panda plugins install @panda/matrix
```

Local checkout (when running from a git repo):

```bash
panda plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `panda plugins install @panda/matrix`
   - From a local checkout: `panda plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Panda calls the same
     login endpoint, stores the access token in `~/.pandabot/credentials/matrix/credentials.json`,
...
- On first connection, Panda requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Panda logs a warning.
...
`~/.pandabot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `panda pairing list matrix`
  - `panda pairing approve matrix <CODE>`
...
- On startup, Panda resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
