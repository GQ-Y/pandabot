...
read_when:
  - You want Panda to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Panda to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`panda onboard`) and `panda channels add` list optional channel plugins.
...
```bash
panda plugins install @panda/nostr
```

Use a local checkout (dev workflows):

```bash
panda plugins install --link <path-to-panda>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "panda@example.com",
        "lud16": "panda@example.com"
      }
    }
  }
}
