...
read_when:
  - Setting up Twitch chat integration for Panda
---
# Twitch (plugin)

Twitch chat support via IRC connection. Panda connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
panda plugins install @panda/twitch
```

Local checkout (when running from a git repo):

```bash
panda plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `PANDABOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "panda",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use PANDABOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
PANDABOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
