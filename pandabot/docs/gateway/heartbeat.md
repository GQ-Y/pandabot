...
- During heartbeat runs, Panda treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `panda sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Panda skips the heartbeat run entirely (no model call).
...
```bash
panda system event --text "Check for urgent follow-ups" --mode now
```
