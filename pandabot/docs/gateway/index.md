...
- Replaces the legacy `gateway` command. CLI entry point: `panda gateway`.
...
pnpm gateway:watch
```
- Config hot reload watches `~/.pandabot/panda.json` (or `PANDABOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__panda__/canvas/` from `~/panda/canvas`. Disable with `canvasHost.enabled=false` or `PANDABOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `PANDABOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `PANDABOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.pandabot.*` may still exist)
- Linux: `panda-gateway-<profile>.service`
- Windows: `Panda Gateway (<profile>)`

Install metadata is embedded in the service config:
- `PANDABOT_SERVICE_MARKER=panda`
- `PANDABOT_SERVICE_KIND=gateway`
- `PANDABOT_SERVICE_VERSION=<version>`
...
```bash
panda --dev setup
panda --dev gateway --allow-unconfigured
...
- `PANDABOT_STATE_DIR=~/.pandabot-dev`
- `PANDABOT_CONFIG_PATH=~/.pandabot-dev/panda.json`
- `PANDABOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/panda-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `PANDABOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `PANDABOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `PANDABOT_CONFIG_PATH`
- unique `PANDABOT_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
panda --profile main gateway install
panda --profile rescue gateway install
```

Example:
```bash
PANDABOT_CONFIG_PATH=~/.pandabot/a.json PANDABOT_STATE_DIR=~/.pandabot-a panda gateway --port 19001
PANDABOT_CONFIG_PATH=~/.pandabot/b.json PANDABOT_STATE_DIR=~/.pandabot-b panda gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `panda health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `panda`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `panda gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.pandabot.*` is cleaned up).
  - `panda doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Panda profile services.
  ...
  - Cleanup: `panda gateway uninstall` (current service) and `panda doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `panda gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Panda.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.pandabot.*` labels still unload cleanly).
- To stop it cleanly, use `panda gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `panda gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `panda gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
Panda installs a **systemd user service** by default on Linux/WSL2. We
...
`panda gateway install` writes the user unit. `panda doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/panda-gateway[-<profile>].service`:
```
[Unit]
Description=Panda Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/panda gateway --port 18789
Restart=always
RestartSec=5
Environment=PANDABOT_GATEWAY_TOKEN=
...
```
systemctl --user enable --now panda-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/panda-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now panda-gateway[-<profile>].service
```
...
## CLI helpers
- `panda gateway health|status` — request health/status over the Gateway WS.
- `panda message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `panda agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `panda gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `panda gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `panda gateway` and the legacy TCP control port.
