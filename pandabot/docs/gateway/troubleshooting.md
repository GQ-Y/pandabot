---
summary: "Quick troubleshooting guide for common Panda failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Panda misbehaves, here's how to fix it.
...
|---|---|---|
| `panda status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `panda status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `panda status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `panda gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `panda channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `panda gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `panda logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `panda status --all` (it redacts tokens). If you paste `panda status`, consider setting `PANDABOT_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  panda models auth setup-token --provider anthropic
  ```
...
```bash
panda models auth paste-token --provider anthropic
panda models status
```
...
**Logs:**
- Preferred: `panda logs --follow`
- File logs (always): `/tmp/panda/panda-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$PANDABOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u panda-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  panda config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  panda config set gateway.mode remote
  panda config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `panda setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.pandabot/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `panda doctor`
...
- then run `panda sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `panda gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `panda gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `panda gateway status` and `panda doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `PANDABOT_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `panda gateway status` shows a config mismatch**
...
- Fix: rerun `panda gateway install --force` from the same `--profile` / `PANDABOT_STATE_DIR` you want the service to use.

**If `panda gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `panda doctor` to update it (or `panda gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `PANDABOT_GATEWAY_TOKEN`) and restart the service.

**If `panda gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/panda` on disk.
...
Symptoms: `pwd` or file tools show `~/.pandabot/sandboxes/...` even though you
...
Panda intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `panda models list` or
  `panda models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${PANDABOT_CONFIG_PATH:-$HOME/.pandabot/panda.json}"
```
...
tail -f "$(ls -t /tmp/panda/panda-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
panda logs --follow | grep "pairing request"
```
...
- ❌ `@panda` + image
- ✅ `@panda check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.pandabot/agents/<agentId>/sessions/
```
...
panda logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
panda gateway --verbose
```

If you’re logged out / unlinked:

```bash
panda channels logout
trash "${PANDABOT_STATE_DIR:-$HOME/.pandabot}/credentials" # if logout can't cleanly remove everything
panda channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/panda/panda-*.log | head -1)" | tail -20
```

### High Memory Usage

Panda keeps conversation history in memory.
...
Panda now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `panda doctor` reports every invalid entry.
- `panda doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `panda logs`, `panda health`, `panda status`, `panda gateway status`, and `panda gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/panda/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `panda models status` (CLI).
...
5) Run `panda channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Panda scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/pandabot/pandabot/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
panda gateway status
panda gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.pandabot.* still works)
```
...
Ensure the global `panda` CLI is installed and matches the app version:
```bash
panda --version
npm install -g panda@<version>
```
...
#   ${PANDABOT_CONFIG_PATH:-$HOME/.pandabot/panda.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
panda gateway --verbose
panda channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/panda/panda-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$PANDABOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.pandabot/logs/...`; profiles use `~/.pandabot-<profile>/logs/...`)<br />Linux: `journalctl --user -u panda-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Panda Gateway (<profile>)" /V /FO LIST` |
| Session files | `$PANDABOT_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$PANDABOT_STATE_DIR/media/` |
| Credentials | `$PANDABOT_STATE_DIR/credentials/` |
...
trash "${PANDABOT_STATE_DIR:-$HOME/.pandabot}"
panda channels login         # re-pair WhatsApp
panda gateway restart           # or: panda gateway
```
...
1. Check logs first: `/tmp/panda/` (default: `panda-YYYY-MM-DD.log`, or your configured `logging.file`)
