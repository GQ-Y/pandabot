---
summary: "All configuration options for ~/.pandabot/panda.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Panda reads an optional **JSON5** config from `~/.pandabot/panda.json` (comments + trailing commas allowed).

If the file is missing, Panda uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.panda`). You usually only need a config to:
...
Panda only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `panda doctor`, `panda logs`, `panda health`, `panda status`, `panda service`, `panda help`).
- Run `panda doctor` to see the exact issues.
- Run `panda doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `panda config set`. Keep a backup of `~/.pandabot/panda.json`.
...
```bash
panda gateway call config.get --params '{}' # capture payload.hash
panda gateway call config.apply --params '{
...
```bash
panda gateway call config.get --params '{}' # capture payload.hash
panda gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.panda" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@panda", "reisponde"] }
      }
    ]
  },
  channels: {
...
```json5
// ~/.pandabot/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [
    { id: "main", workspace: "~/.panda" }
  ]
}
```
...
{ "$include": "/etc/panda/base.json5" } // absolute
...
```json5
// ~/.pandabot/clients/mueller/agents.json5
[
...
```json5
// ~/.pandabot/clients/mueller/broadcast.json5
{
...
Panda reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Panda runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `PANDABOT_LOAD_SHELL_ENV=1`
- `PANDABOT_SHELL_ENV_TIMEOUT_MS=15000`
...
Panda stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.pandabot/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.pandabot/credentials/oauth.json` (or `$PANDABOT_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.pandabot/agent/*` (migrated by `panda doctor` into `~/.pandabot/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `PANDABOT_OAUTH_DIR`
- Agent dir (default agent root override): `PANDABOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Panda imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Panda derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/panda/panda-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/panda/panda.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.pandabot/credentials/whatsapp/biz
          // authDir: "~/.pandabot/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `panda doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@panda", "panda"] } }
    ]
  }
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@panda"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.panda-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.pandabot/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.panda-home" },
      { id: "work", workspace: "~/.panda-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `panda.json`).
...
Panda starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["panda-dm"]          // optional group DM allowlist
      },
      guilds: {
...
Panda starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `panda plugins install @panda/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Panda starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Panda spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Panda
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/panda" } }
}
```
...
When a file exceeds this limit, Panda logs a warning and injects a truncated
...
message envelopes). If unset, Panda uses the host timezone at runtime.
...
`[panda]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[panda]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[panda]"`, Panda will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Panda generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.pandabot/settings/tts.json",
      elevenlabs: {
...
Panda also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Panda currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `panda configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:panda`: all built-in Panda tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.pandabot/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "panda-sandbox-browser:bookworm-slim",
          containerPrefix: "panda-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Panda uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.pandabot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Panda config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Panda writes/merges a `models.json` into
`~/.pandabot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Panda uses
...
- Shortcut: `panda onboard --auth-choice opencode-zen`.
...
Shortcut: `panda onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `panda onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `panda onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `panda onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `panda onboard --auth-choice minimax-api`.
...
- Override the agent config root with `PANDABOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.pandabot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.pandabot/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.pandabot/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Panda stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.pandabot/skills` (workspace skills
...
from `~/.pandabot/extensions`, `<workspace>/.pandabot/extensions`, plus any
...
### `browser` (panda-managed browser)

Panda can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for panda and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Panda.app menubar, or `panda gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      panda: { cdpPort: 18800, color: "#FF4500" },
...
- When a connection comes from one of these IPs, Panda uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `panda gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `PANDABOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `PANDABOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Panda
...
macOS app behavior:
- Panda.app watches `~/.pandabot/panda.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.pandabot/panda.json` (or `PANDABOT_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.pandabot/panda.json` (or `PANDABOT_CONFIG_PATH`)
...
- `PANDABOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `panda --dev …` → uses `~/.pandabot-dev` + shifts ports from base `19001`
- `panda --profile <name> …` → uses `~/.pandabot-<name>` (port via config/env/flags)
...
Gmail helper config (used by `panda webhooks gmail setup` / `run`):
...
- Set `PANDABOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Panda defaults `serve.path` to `/` so
...
Default root: `~/.panda/canvas`  
Default port: `18793` (chosen to avoid the panda browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__panda/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__panda__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `PANDABOT_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.pandabot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.pandabot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.pandabot/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_panda-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_panda-bridge._tcp` under `~/.pandabot/dns/` using the standard discovery domain `panda.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `panda.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `panda.internal` via that server
