---
summary: "Panda on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Panda on a Raspberry Pi
  - Running Panda on ARM devices
  - Building a cheap always-on personal AI
---

# Panda on Raspberry Pi

## Goal

Run a persistent, always-on Panda Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Panda features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled panda
...
```bash
# Check logs
journalctl -u panda --no-pager -n 100

# Common fix: rebuild
cd ~/panda  # if using hackable install
npm run build
sudo systemctl restart panda
```
