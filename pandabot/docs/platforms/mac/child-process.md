...
service via the external `panda` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`PANDABOT_PROFILE`; legacy `com.pandabot.*` is supported).
