...
read_when:
  - You are building a Panda plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (panda.plugin.json)

Every plugin **must** ship a `panda.plugin.json` file in the **plugin root**.
Panda uses this manifest to validate configuration **without executing plugin
