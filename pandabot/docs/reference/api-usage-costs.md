...
Panda features that can generate provider usage or paid API calls.
...
- `panda status --usage` and `panda channels list` show provider **usage windows**
...
Panda can pick up credentials from:
...
### 8) Model scan / probe
`panda models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
