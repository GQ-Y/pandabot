---
summary: "Use Venice AI privacy-focused models in Panda"
read_when:
  - You want privacy-focused inference in Panda
...
```bash
panda onboard --auth-choice venice-api-key
```
...
```bash
panda chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Panda shows all available Venice models. Pick based on your needs:
...
```bash
panda models set venice/claude-opus-45
panda models set venice/llama-3.3-70b
```

List all available models:

```bash
panda models list | grep venice
```

## Configure via `panda configure`

1. Run `panda configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Panda automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
panda chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
panda chat --model venice/claude-opus-45

# Use uncensored model
panda chat --model venice/venice-uncensored

# Use vision model with image
panda chat --model venice/qwen3-vl-235b-a22b

# Use coding model
panda chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `panda models list` to see currently available models. Some models may be temporarily offline.
