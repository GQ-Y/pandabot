---
summary: "Model providers (LLMs) supported by Panda"
read_when:
...
Panda can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `panda onboard`).
