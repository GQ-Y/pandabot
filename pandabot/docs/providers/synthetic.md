---
summary: "Use Synthetic's Anthropic-compatible API in Panda"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Panda registers it as the
...
```bash
panda onboard --auth-choice synthetic-api-key
```
...
Note: Panda's Anthropic client appends `/v1` to the base URL, so use
