...
read_when:
  - Designing workspace memory (~/panda) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Panda integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/panda`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/panda` as canonical human-readable memory.
...
Recommendation: **deep integration in Panda**, but keep a separable core library.

### Why integrate into Panda?
- Panda already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `panda memory recall "…" --k 25 --since 30d`
  - `panda memory reflect --since 7d`
