---
summary: "Chrome extension: let Panda drive your existing Chrome tab"
read_when:
...
The Panda Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate panda-managed Chrome profile.
...
Panda then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Panda release (npm package) as static files. There is no separate “build” step.

After upgrading Panda:
- Re-run `panda browser extension install` to refresh the installed files under your Panda state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Panda ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Panda to control.
...
- `ON`: attached; Panda can drive that tab.
...
Debugging: `panda sandbox explain`
...
`panda browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `panda browser extension install` first to copy the extension to a stable location under your Panda state directory.
...
- **This is not isolated** like the dedicated panda-managed profile.
