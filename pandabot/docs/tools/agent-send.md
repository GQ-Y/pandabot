---
summary: "Direct `panda agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `panda agent` (direct agent runs)

`panda agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `panda message --target`).
...
```bash
panda agent --to +15555550123 --message "status update"
panda agent --agent ops --message "Summarize logs"
...
panda agent --to +15555550123 --message "Summon reply" --deliver
