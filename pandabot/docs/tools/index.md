---
summary: "Agent tool surface for Panda (browser, canvas, nodes, message, cron) replacing legacy `panda-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `panda-*` skills
---

# Tools (Panda)

Panda exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `panda-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `panda.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Panda logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`panda node run`).
...
Notes:
- Requires a Brave API key (recommended: `panda configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated panda browser.
...
- Quick smoke: `panda nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `panda gateway` restart in-place)
