---
summary: "First-run onboarding flow for Panda (macOS app)"
read_when:
...
- Writes credentials to `~/.pandabot/credentials/oauth.json`
...
The app can install the global `panda` CLI via npm/pnpm so terminal
...
On the first agent run, Panda bootstraps a workspace (default `~/panda`):
...
```bash
panda webhooks gmail setup --account you@gmail.com
```
...
- `~/.pandabot/credentials/oauth.json`
