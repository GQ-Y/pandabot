...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `panda sandbox explain`.
...
Legacy `agent.*` configs are migrated by `panda doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=panda.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${PANDABOT_STATE_DIR:-$HOME/.pandabot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
