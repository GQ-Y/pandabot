...
Deploy Panda on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/panda/panda" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run Panda:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: PANDABOT_STATE_DIR
        value: /data/.pandabot
      - key: PANDABOT_WORKSPACE_DIR
        value: /data/workspace
      - key: PANDABOT_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/panda`.
...
If you use the original Panda repository, Render will not auto-deploy your Panda. To update it, run a manual Blueprint sync from the dashboard.
...
For Panda, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Panda host.
