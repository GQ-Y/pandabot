...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Panda
---

# Node + tsx "__name is not a function" crash

## Summary
Running Panda via Node with `tsx` fails at startup with:

```
[panda] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `panda status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch panda.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node panda.mjs status` works on Node 25.
