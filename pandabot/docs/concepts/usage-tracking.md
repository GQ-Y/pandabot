...
- `/usage cost` in chats: local cost summary aggregated from Panda session logs.
- CLI: `panda status --usage` prints a full per-provider breakdown.
- CLI: `panda channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
