...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when Panda would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When Panda would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/panda-alfred/
...
Workspace: /Users/pascal/panda-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.pandabot/logs/gateway.log | grep broadcast
```
