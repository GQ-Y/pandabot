...
Panda ships two installer scripts (served from `molt.bot`):
...
If the installer completes but `panda` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- Choose install method:
  - `npm` (default): `npm install -g panda@latest`
...
- If upgrading an existing install: runs `panda doctor --non-interactive` (best effort).
- For git installs: runs `panda doctor --non-interactive` after install/update (best effort).
...
If you run the installer while **already inside a Panda source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:

- update and use this checkout (`git`)
- or migrate to the global npm install (`npm`)

In non-interactive contexts (no TTY / `--no-prompt`), you must pass `--install-method git|npm` (or set `PANDABOT_INSTALL_METHOD`), otherwise the script exits with code `2`.
...
This script installs `panda` into a prefix (default: `~/.pandabot`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.
...
- Choose install method:
  - `npm` (default): `npm install -g panda@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `panda doctor --non-interactive` on upgrades and git installs (best effort).
...
```powershell
iwr -useb https://molt.bot/install.ps1 | iex -InstallMethod git -GitDir "C:\\panda"
```

Environment variables:

- `PANDABOT_INSTALL_METHOD=git|npm`
- `PANDABOT_GIT_DIR=...`
...
- **"panda" is not recognized**: your npm global bin folder is not on PATH. Most systems use
