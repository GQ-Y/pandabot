...
read_when:
  - "You installed Panda but `panda` is “command not found”"
...
Panda’s runtime baseline is **Node 22+**.

If you can run `npm install -g panda@latest` but later see `panda: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
