...
Panda can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Panda can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Panda defaults
...
TTS config lives under `messages.tts` in `panda.json`.
...
- If `provider` is **unset**, Panda prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.pandabot/settings/tts.json`, override with `PANDABOT_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Panda retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Panda registers
