...
Deploy Panda on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/panda-railway-template" target="_blank" rel="noreferrer">Deploy on Railway</a>
...
- `https://<your-railway-domain>/panda` — Control UI

## What you get

- Hosted Panda Gateway + Control UI
...
- `PANDABOT_STATE_DIR=/data/.pandabot` (recommended)
- `PANDABOT_WORKSPACE_DIR=/data/workspace` (recommended)
- `PANDABOT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Panda state + workspace so you can migrate to another host without losing config or memory.
