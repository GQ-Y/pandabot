...
- Jobs persist under `~/.pandabot/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.pandabot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.pandabot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
panda cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
panda cron edit <jobId> --agent ops
panda cron edit <jobId> --clear-agent
```
```
...
```bash
panda system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`panda system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `PANDABOT_SKIP_CRON`.
