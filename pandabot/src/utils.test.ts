...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "panda-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "panda-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "panda-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "panda-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.panda when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "panda-config-dir-"));
    try {
      const newDir = path.join(root, ".panda");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/panda")).toBe(path.resolve(os.homedir(), "panda"));
  });
