...
import { scheduleGatewaySigusr1Restart, triggerPandaRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting panda in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerPandaRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting panda via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
