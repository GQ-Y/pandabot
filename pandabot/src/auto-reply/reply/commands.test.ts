...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-commands-"));
...
function buildParams(commandBody: string, cfg: PandaConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
