...
type AgentConfig = Partial<NonNullable<NonNullable<PandaConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.panda/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🐼 Pandabot ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: PandaConfig): string {
  const lines = ["ℹ️ Help", ""];
