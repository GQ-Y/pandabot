...
describe("initSessionState sender meta", () => {
  it("injects sender meta into BodyStripped for group chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "panda-sender-meta-"));
  ...
  it("does not inject sender meta for direct chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "panda-sender-meta-direct-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as PandaConfig;
...
describe("mention helpers", () => {
  it("builds regexes and skips invalid patterns", () => {
    const regexes = buildMentionRegexes({
      messages: {
        groupChat: { mentionPatterns: ["\\bpanda\\b", "(?"] },
      },
    });
    expect(regexes).toHaveLength(1);
    expect(regexes[0]?.test("panda")).toBe(true);
  });

  it("normalizes zero-width characters", () => {
    expect(normalizeMentionText("pan\u200bda")).toBe("panda");
  });

  it("matches patterns case-insensitively", () => {
    const regexes = buildMentionRegexes({
      messages: { groupChat: { mentionPatterns: ["\\bpanda\\b"] } },
    });
    expect(matchesMentionPatterns("PANDA: hi", regexes)).toBe(true);
  });
