import type { PandaConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: PandaConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: PandaConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: PandaConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: PandaConfig) {
