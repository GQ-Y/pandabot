...
vi.mock("./models-config.js", () => ({
  ensurePandaModelsJson: vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false }),
}));

vi.mock("./agent-paths.js", () => ({
  resolvePandaAgentDir: () => "/tmp/panda",
}));
...
describe("loadModelCatalog", () => {
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as PandaConfig });
