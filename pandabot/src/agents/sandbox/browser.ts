...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_PANDA_BROWSER_COLOR,
    ...
    profiles: {
      panda: { cdpPort: params.cdpPort, color: DEFAULT_PANDA_BROWSER_COLOR },
    },
  };
}
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    args.push("-e", `PANDABOT_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `PANDABOT_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `PANDABOT_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `PANDABOT_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `PANDABOT_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing ? resolveProfile(existing.bridge.state.resolved, "panda") : null;
  const shouldReuse =
