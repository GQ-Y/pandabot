...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "panda.sandbox=1");
  args.push("--label", `panda.sessionKey=${params.scopeKey}`);
  args.push("--label", `panda.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `panda.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "panda.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`panda sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`panda sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("panda sandbox recreate --all");
}
