...
const defaultTools = createPandaCodingTools();

describe("createPandaCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createPandaCodingTools();
    ...
    try {
      ...
      const contents = "Hello from panda read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "panda-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "panda-workspace"),
    ...
    const tools = createPandaCodingTools({ sandbox });
