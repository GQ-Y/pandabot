...
import { __testing, createPandaCodingTools } from "./pi-tools.js";
...
describe("createPandaCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createPandaCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "panda-outside.txt");
