...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolvePandaAgentDir() returns mainAgentDir
    process.env.PANDA_STATE_DIR = tmpDir;
    process.env.PANDA_AGENT_DIR = mainAgentDir;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.PANDA_AGENT_DIR;
    else process.env.PANDA_AGENT_DIR = previousAgentDir;
