...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    ...
    const previousAgentDir = process.env.PANDA_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-oauth-"));

    try {
      process.env.PANDA_STATE_DIR = tempDir;
      process.env.PANDA_AGENT_DIR = path.join(tempDir, "agent");
      ...
      const store = ensureAuthProfileStore(process.env.PANDA_AGENT_DIR, {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.PANDA_STATE_DIR = previousStateDir;
      }
      if (previousAgentDir === undefined) {
        delete process.env.PANDA_AGENT_DIR;
      } else {
        process.env.PANDA_AGENT_DIR = previousAgentDir;
      }
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-auth-"));
