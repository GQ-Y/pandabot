...
/**
 * Cleanup suspended Panda CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: PandaConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-cli-images-"));
  const paths: string[] = [];
