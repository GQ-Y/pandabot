...
type WebSearchConfig = NonNullable<PandaConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: PandaConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("panda configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.pandabot.cc/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://pandabot.cc",
      "X-Title": "Pandabot Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
