...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "panda-models-" });
}

const MODELS_CONFIG: PandaConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensurePandaModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensurePandaModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensurePandaModelsJson } = await import("./models-config.js");
      const { resolvePandaAgentDir } = await import("./agent-paths.js");

      await ensurePandaModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolvePandaAgentDir(), "models.json");
  ...
  it("adds minimax provider when MINIMAX_API_KEY is set", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        expect(parsed.providers.minimax?.baseUrl).toBe("https://api.minimaxi.com/v1");
