...
`;
    ...
    expect(parsed.panda.emoji).toBe("💾");
    expect(parsed.panda.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.panda.emoji).toBe("📝");
    expect(parsed.panda.events).toEqual(["command"]);
    expect(parsed.panda.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.panda.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"panda": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"panda": {"events": ["test"]}}');
  });
...
describe("resolvePandaMetadata", () => {
  it("extracts panda metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolvePandaMetadata(frontmatter);
  ...
  it("returns undefined when panda key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        panda: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Panda" },
            { id: "npm", kind: "npm", package: "@pandabot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@pandabot/hook");
  });
    ...
    "panda":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Panda" }],
      },
  }
---
...
`;
    ...
    const meta = resolvePandaMetadata(frontmatter);
    expect(meta).toBeDefined();
    expect(meta?.emoji).toBe("💾");
    expect(meta?.events).toEqual(["command:new"]);
    expect(meta?.requires?.config).toEqual(["workspace.dir"]);
    expect(meta?.install?.[0].kind).toBe("bundled");
  });
