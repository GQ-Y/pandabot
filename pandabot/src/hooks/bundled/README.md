...
This directory contains hooks that ship with Pandabot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
```bash
pandabot hooks enable session-memory
```
...
```bash
pandabot hooks enable command-logger
```
...
**Docs**: https://docs.pandabot.cc/hooks/soul-evil

**Enable**:

```bash
pandabot hooks enable soul-evil
```
...
homepage: https://docs.pandabot.cc/hooks#my-hook
metadata:
  { "pandabot": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
```bash
pandabot hooks info session-memory
```

Check hook status:

```bash
pandabot hooks check
```
...
```bash
pandabot hooks enable session-memory
pandabot hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.pandabot/pandabot.json`:
...
2. Restart gateway: `pkill -9 -f 'pandabot.*gateway' && pnpm pandabot gateway`
3. Enable the hook: `pandabot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.pandabot.cc/hooks
