...
homepage: https://docs.pandabot.cc/hooks#command-logger
metadata:
  {
    "panda":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.pandabot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.pandabot/logs/commands.log ~/.pandabot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/panda`:
   ```
   /home/username/.pandabot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
