...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { panda?: { emoji?: string } };
    expect(parsed.panda?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { panda?: { events?: string[] } };
