...
import { resolvePandaAgentDir } from "../agents/agent-paths.js";
...
import { ensurePandaModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { PandaConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.PANDA_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.PANDA_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.PANDA_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<PandaConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.PANDA_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.PANDA_STATE_DIR,
  };
  ...
  const hostAgentDir = resolvePandaAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-live-state-"));
  ...
  const toolProbePath = path.join(workspaceDir, `.pandabot-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolvePandaAgentDir();
  const sanitizedCfg: PandaConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Panda live tool probe (local, safe): " +
    ...
    process.env.PANDA_CONFIG_PATH = previous.configPath;
    process.env.PANDA_GATEWAY_TOKEN = previous.token;
    process.env.PANDA_SKIP_CHANNELS = previous.skipChannels;
    process.env.PANDA_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.PANDA_SKIP_CRON = previous.skipCron;
    process.env.PANDA_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.PANDA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.PANDA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensurePandaModelsJson(cfg);
      ...
      const rawModels = process.env.PANDA_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.pandabot-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
