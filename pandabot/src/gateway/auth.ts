...
export function resolveGatewayAuth(params: {
  ...
  const token = authConfig.token ?? env.PANDA_GATEWAY_TOKEN ?? undefined;
  const password = authConfig.password ?? env.PANDA_GATEWAY_PASSWORD ?? undefined;
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or PANDA_GATEWAY_TOKEN)",
    );
  }
