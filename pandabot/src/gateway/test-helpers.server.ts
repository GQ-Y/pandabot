...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.PANDA_STATE_DIR;
  previousConfigPath = process.env.PANDA_CONFIG_PATH;
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.PANDA_STATE_DIR;
    else process.env.PANDA_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.PANDA_CONFIG_PATH;
    else process.env.PANDA_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.PANDA_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.PANDA_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.PANDA_SKIP_GMAIL_WATCHER;
    else process.env.PANDA_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.PANDA_SKIP_CANVAS_HOST;
    else process.env.PANDA_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.PANDA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.PANDA_GATEWAY_TOKEN = fallbackToken;
  }
