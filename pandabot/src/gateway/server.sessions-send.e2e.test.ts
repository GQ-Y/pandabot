...
import { createPandaTools } from "../agents/panda-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.PANDA_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.PANDA_GATEWAY_PORT = String(gatewayPort);
  process.env.PANDA_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.PANDA_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createPandaTools().find((candidate) => candidate.name === "sessions_send");
