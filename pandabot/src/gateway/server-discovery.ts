...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Panda";
  if (/panda/i.test(trimmed)) return trimmed;
  return `${trimmed} (Panda)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.PANDABOT_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "panda");
  ...
  const binCli = path.join(cwd, "bin", "panda.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.PANDABOT_TAILNET_DNS?.trim();
