...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("pandabotCanvasA2UIAction");
    expect(out).toContain("pandaSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("pandaSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.pandaA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__pandabot__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("panda-a2ui-host");
      expect(html).toContain("pandabotCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__pandabot__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("pandaA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
