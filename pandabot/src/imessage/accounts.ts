...
export function listIMessageAccountIds(cfg: PandaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: PandaConfig): string {
...
function mergeIMessageAccountConfig(cfg: PandaConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: PandaConfig): ResolvedIMessageAccount[] {
