...
export const WIDE_AREA_DISCOVERY_DOMAIN = "panda.internal.";
export const WIDE_AREA_ZONE_FILENAME = "panda.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*panda-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "panda";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "panda");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "panda-gw");
  ...
  records.push(`_panda-gw._tcp IN PTR ${instanceLabel}._panda-gw._tcp`);
  records.push(`${instanceLabel}._panda-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._panda-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; panda-content-hash: ${contentHash}\n${contentBody}`;
}
