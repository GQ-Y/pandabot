...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.PANDA_DISABLE_BONJOUR)) return true;
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Panda";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Panda\)\s*$/i, "").trim() || normalized;
}
