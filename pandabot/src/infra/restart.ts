...
export function triggerPandaRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.PANDA_SYSTEMD_UNIT,
        process.env.PANDA_PROFILE,
      );
  ...
  const label =
    process.env.PANDA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.PANDA_PROFILE);
