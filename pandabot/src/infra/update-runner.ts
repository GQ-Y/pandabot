...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "pandabot-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "panda doctor",
        managerScriptArgs(manager, "panda", ["doctor", "--non-interactive"]),
        gitRoot,
        { PANDA_UPDATE_IN_PROGRESS: "1" },
      ),
    );
