...
describe("doctor command", () => {
  ...
  it("warns when extra workspace directories exist", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      ...
      valid: true,
      config: {
        agents: { defaults: { workspace: "/Users/steipete/panda" } },
      },
      issues: [],
      legacyIssues: [],
    });
    ...
    expect(
      note.mock.calls.some(
        ([message, title]) =>
          title === "Extra workspace" && typeof message === "string" && message.includes("panda"),
      ),
    ).toBe(true);
