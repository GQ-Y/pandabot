...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.PANDA_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.PANDA_UPDATE_IN_PROGRESS;
  } else {
    process.env.PANDA_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "panda-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.PANDA_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when PANDA_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/panda.json",
      exists: true,
      raw: "{}",
    ...
    process.env.PANDA_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
