...
function resolveMode(cfg: PandaConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Pandabot doctor");

  const root = await resolvePandaPackageRoot({
  ...
  let cfg: PandaConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("panda configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("panda config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("panda setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("panda doctor --fix")}" to apply changes.`);
  }
