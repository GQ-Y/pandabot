...
export function listAgentEntries(cfg: PandaConfig): AgentEntry[] {
...
function resolveAgentName(cfg: PandaConfig, agentId: string) {
...
function resolveAgentModel(cfg: PandaConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: PandaConfig): AgentSummary[] {
