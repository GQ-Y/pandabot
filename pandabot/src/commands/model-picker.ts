...
function resolveConfiguredModelRaw(cfg: PandaConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: PandaConfig): string[] {
...
export function applyPrimaryModel(cfg: PandaConfig, model: string): PandaConfig {
...
export function applyModelAllowlist(cfg: PandaConfig, models: string[]): PandaConfig {
  const defaults = cfg.agents?.defaults;
