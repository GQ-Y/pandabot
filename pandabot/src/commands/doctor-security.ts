...
import type { PandaConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: PandaConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("panda security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("panda configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("panda config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("panda doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("panda config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
