...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.PANDABOT_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: PandaConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.PANDABOT_DEBUG_HEALTH);
    if (opts.verbose) {
