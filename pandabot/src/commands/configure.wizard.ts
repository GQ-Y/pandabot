  ...
  try {
    ...
    if (opts.sections) {
      ...
      if (selected.includes("health")) {
        ...
        const token = nextConfig.gateway?.auth?.token ?? process.env.PANDA_GATEWAY_TOKEN;
        const password =
          nextConfig.gateway?.auth?.password ?? process.env.PANDA_GATEWAY_PASSWORD;
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.PANDA_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.PANDA_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.PANDA_GATEWAY_TOKEN;
