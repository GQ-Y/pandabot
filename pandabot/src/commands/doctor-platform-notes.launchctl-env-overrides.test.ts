...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "PANDA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("PANDA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv PANDA_GATEWAY_TOKEN");
    expect(message).not.toContain("PANDA_GATEWAY_PASSWORD");
  });
