...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const envToken = await getenv("PANDA_GATEWAY_TOKEN");
  const envPassword = await getenv("PANDA_GATEWAY_PASSWORD");
  if (!envToken && !envPassword) return;

  const lines = [
    "- launchctl environment overrides detected (can cause confusing unauthorized errors).",
    envToken ? "- `PANDA_GATEWAY_TOKEN` is set; it overrides config tokens." : undefined,
    envPassword
      ? "- `PANDA_GATEWAY_PASSWORD` is set; it overrides config passwords."
      : undefined,
    "- Clear overrides and restart the app/gateway:",
    envToken ? "  launchctl unsetenv PANDA_GATEWAY_TOKEN" : undefined,
    envPassword ? "  launchctl unsetenv PANDA_GATEWAY_PASSWORD" : undefined,
  ].filter((line): line is string => Boolean(line));
