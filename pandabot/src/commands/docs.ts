...
const SEARCH_TOOL = "https://docs.pandabot.dev/mcp.SearchPandabot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.pandabot.cc");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('panda docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.pandabot.cc/");
      runtime.log(`Search: ${formatCliCommand('panda docs "your query"')}`);
    }
    return;
  }
