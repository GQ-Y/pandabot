...
function resolveDefaultAccountId(cfg: PandaConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: PandaConfig): boolean {
...
export function summarizeBindings(cfg: PandaConfig, bindings: AgentBinding[]): string[] {
