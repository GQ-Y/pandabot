...
import { readConfigFileSnapshot, type PandaConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: PandaConfig, model: string): PandaConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("panda plugins install")}\`.`,
    );
  }
