...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvePandaAgentDir: vi.fn().mockReturnValue("/tmp/panda-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/panda-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolvePandaAgentDir: mocks.resolvePandaAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/panda-agent/auth-profiles.json");
