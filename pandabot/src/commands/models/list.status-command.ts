...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("panda models auth setup-token")}\` or \`${formatCliCommand("panda configure")}\`.`
          : `Run \`${formatCliCommand("panda configure")}\` or set an API key env var.`;
