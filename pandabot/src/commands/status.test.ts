...
beforeAll(() => {
  previousProfile = process.env.PANDA_PROFILE;
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.PANDA_PROFILE = previousProfile;
  }
});
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Pandabot status"))).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.PANDA_GATEWAY_TOKEN = "abcd1234";
    try {
