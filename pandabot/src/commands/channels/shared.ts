...
import { type PandaConfig, readConfigFileSnapshot } from "../../config/config.js";
...
export async function requireValidConfig(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("panda doctor")}.`);
    runtime.exit(1);
    return null;
  }
