...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("panda node install"), formatCliCommand("panda node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("panda node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    PANDA_LOG_PREFIX: baseEnv.PANDA_LOG_PREFIX ?? "node",
