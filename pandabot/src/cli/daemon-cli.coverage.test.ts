...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    PANDA_STATE_DIR: process.env.PANDA_STATE_DIR,
    PANDA_CONFIG_PATH: process.env.PANDA_CONFIG_PATH,
    PANDA_GATEWAY_PORT: process.env.PANDA_GATEWAY_PORT,
    PANDA_PROFILE: process.env.PANDA_PROFILE,
  };
  ...
  afterEach(() => {
    if (originalEnv.PANDA_STATE_DIR !== undefined)
      process.env.PANDA_STATE_DIR = originalEnv.PANDA_STATE_DIR;
    else delete process.env.PANDA_STATE_DIR;

    if (originalEnv.PANDA_CONFIG_PATH !== undefined)
      process.env.PANDA_CONFIG_PATH = originalEnv.PANDA_CONFIG_PATH;
    else delete process.env.PANDA_CONFIG_PATH;

    if (originalEnv.PANDA_GATEWAY_PORT !== undefined)
      process.env.PANDA_GATEWAY_PORT = originalEnv.PANDA_GATEWAY_PORT;
    else delete process.env.PANDA_GATEWAY_PORT;

    if (originalEnv.PANDA_PROFILE !== undefined)
      process.env.PANDA_PROFILE = originalEnv.PANDA_PROFILE;
    else delete process.env.PANDA_PROFILE;
  });
