...
import { resolvePandaPackageRoot } from "../infra/panda-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "panda doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "panda";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "pandabot"]);
const CLI_NAME = resolveCliName();
const PANDA_REPO_URL = "https://github.com/pandabot/pandabot.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "pandabot");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("panda@")) return trimmed.slice("panda@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.PANDA_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", PANDA_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `PANDA_GIT_DIR points at a non-git directory: ${params.dir}. Set PANDA_GIT_DIR to an empty folder or a pandabot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`PANDA_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Pandabot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Pandabot..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Pandabot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("panda doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("panda gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g panda@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g panda@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("panda gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("panda doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("panda gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("panda gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `panda update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `PANDA_GIT_DIR points at a non-git directory: ${gitDir}. Set PANDA_GIT_DIR to an empty folder or a pandabot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via PANDA_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Pandabot to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["panda update", "Update a source checkout (git)"],
        ["panda update --channel beta", "Switch to beta channel (git + npm)"],
        ["panda update --channel dev", "Switch to dev channel (git + npm)"],
        ["panda update --tag beta", "One-off update to a dist-tag or version"],
        ["panda update --no-restart", "Update without restarting the service"],
        ["panda update --json", "Output result as JSON"],
        ["panda update --yes", "Non-interactive (accept downgrade prompts)"],
        ["panda update wizard", "Interactive update wizard"],
        ["panda --update", "Shorthand for panda update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run panda update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.pandabot.cc/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["panda update status", "Show channel + version status."],
          ["panda update status --json", "JSON output."],
          ["panda update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.pandabot.cc/cli/update")}`,
    )
    .action(async (opts) => {
      try {
