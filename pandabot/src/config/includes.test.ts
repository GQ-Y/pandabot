...
const ETC_PANDABOT_DIR = path.join(ROOT_DIR, "etc", "panda");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "panda.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcPandaPath(...parts: string[]) {
  return path.join(ETC_PANDABOT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcPandaPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "panda.json"))).toEqual({
      shared: true,
    });
  });
});
