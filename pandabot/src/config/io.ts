...
import type { PandaConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { comparePandaVersions } from "./version.js";
...
function coerceConfig(value: unknown): PandaConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as PandaConfig;
}
...
function stampConfigVersion(cfg: PandaConfig): PandaConfig {
...
function warnIfConfigFromFuture(cfg: PandaConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = comparePandaVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Pandabot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: PandaConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): PandaConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as PandaConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as PandaConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: PandaConfig) {
    clearConfigCache();
...
// module scope. `PANDA_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.PANDABOT_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.PANDABOT_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): PandaConfig {
...
export async function writeConfigFile(cfg: PandaConfig): Promise<void> {
  clearConfigCache();
