...
import type { PandaConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: PandaConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: PandaConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = PandaSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as PandaConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as PandaConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as PandaConfig)),
    ),
  };
}
