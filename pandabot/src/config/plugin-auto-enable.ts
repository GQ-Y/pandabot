import type { PandaConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: PandaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: PandaConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: PandaConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: PandaConfig): boolean {
...
function isIMessageConfigured(cfg: PandaConfig): boolean {
...
function isWhatsAppConfigured(cfg: PandaConfig): boolean {
...
function isGenericChannelConfigured(cfg: PandaConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: PandaConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: PandaConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: PandaConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: PandaConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: PandaConfig, pluginId: string): PandaConfig {
...
function enablePluginEntry(cfg: PandaConfig, pluginId: string): PandaConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
