...
describe("Nix integration (U3, U5, U9)", () => {
  describe("U3: isNixMode env var detection", () => {
    it("isNixMode is false when PANDA_NIX_MODE is not set", async () => {
      await withEnvOverride({ PANDA_NIX_MODE: undefined }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when PANDA_NIX_MODE is empty", async () => {
      await withEnvOverride({ PANDA_NIX_MODE: "" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is false when PANDA_NIX_MODE is not '1'", async () => {
      await withEnvOverride({ PANDA_NIX_MODE: "true" }, async () => {
        const { isNixMode } = await import("./config.js");
        expect(isNixMode).toBe(false);
      });
    });

    it("isNixMode is true when PANDA_NIX_MODE=1", async () => {
      await withEnvOverride({ PANDA_NIX_MODE: "1" }, async () => {
        const { isNixMode } = await import("./config.js");
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    ...
    it("STATE_DIR respects PANDA_STATE_DIR override", async () => {
      await withEnvOverride(
        { PANDA_STATE_DIR: undefined, PANDA_STATE_DIR: "/custom/state/dir" },
        async () => {
          const { STATE_DIR } = await import("./config.js");
  ...
  describe("U6: gateway port resolution", () => {
    it("uses default when env and config are unset", async () => {
      await withEnvOverride({ PANDA_GATEWAY_PORT: undefined }, async () => {
        const { DEFAULT_GATEWAY_PORT, resolveGatewayPort } = await import("./config.js");
        expect(resolveGatewayPort({})).toBe(DEFAULT_GATEWAY_PORT);
      });
    });

    it("prefers PANDA_GATEWAY_PORT over config", async () => {
      await withEnvOverride({ PANDA_GATEWAY_PORT: "19001" }, async () => {
    ...
    it("falls back to config when env is invalid", async () => {
      await withEnvOverride({ PANDA_GATEWAY_PORT: "nope" }, async () => {
        const { resolveGatewayPort } = await import("./config.js");
