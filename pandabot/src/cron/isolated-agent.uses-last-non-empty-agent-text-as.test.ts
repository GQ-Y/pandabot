...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "panda-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".pandabot", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<PandaConfig> = {},
): PandaConfig {
  const base: PandaConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".pandabot", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
