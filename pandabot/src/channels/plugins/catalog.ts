...
import { discoverPandaPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { PandaPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["PANDA_PLUGIN_CATALOG_PATHS", "PANDA_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<PandaPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packagePanda?: PandaPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packagePanda;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.panda;
  return buildCatalogEntry({
    packageName: entry.name,
    packagePanda: manifest as PandaPackageManifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverPandaPlugins({ workspaceDir: options.workspaceDir });
