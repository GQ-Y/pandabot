...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `panda-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.panda/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "0.0.1",
        panda: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
...
name: "@pandabot/voice-call",
  ...
  it("rejects packages without panda.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@pandabot/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    if (result.ok) return;
    expect(result.error).toContain("panda.extensions");
  });
});
