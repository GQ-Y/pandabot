...
import { loadPandaPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.PANDA_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `panda-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "pandabot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.PANDA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadPandaPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.PANDA_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadPandaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadPandaPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@pandabot/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        panda: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
