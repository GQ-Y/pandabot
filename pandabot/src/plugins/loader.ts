...
import { discoverPandaPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: PandaPluginDefinition;
  register?: PandaPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as PandaPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as PandaPluginDefinition;
...
export function loadPandaPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverPandaPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "pandabot/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: PandaPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as PandaPluginModule;
    } catch (err) {
