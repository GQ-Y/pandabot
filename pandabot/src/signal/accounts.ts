...
export function listSignalAccountIds(cfg: PandaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: PandaConfig): string {
...
function mergeSignalAccountConfig(cfg: PandaConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: PandaConfig): ResolvedSignalAccount[] {
