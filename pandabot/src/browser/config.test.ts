...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const panda = resolveProfile(resolved, "panda");
    expect(panda?.driver).toBe("panda");
    expect(panda?.cdpPort).toBe(18800);
    expect(panda?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from PANDA_GATEWAY_PORT when unset", () => {
    const prev = process.env.PANDA_GATEWAY_PORT;
    process.env.PANDA_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const panda = resolveProfile(resolved, "panda");
      expect(panda?.cdpPort).toBe(19012);
      expect(panda?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.PANDA_GATEWAY_PORT;
      } else {
        process.env.PANDA_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const panda = resolveProfile(resolved, "panda");
      expect(panda?.cdpPort).toBe(19022);
      expect(panda?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.PANDA_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "panda");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        panda: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("panda");
  });
});
