import type { BrowserConfig, BrowserProfileConfig, PandaConfig } from "../config/config.js";
import {
...
import {
  DEFAULT_PANDA_BROWSER_COLOR,
  DEFAULT_PANDA_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_PANDA_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) return DEFAULT_PANDA_BROWSER_COLOR;
  const normalized = value.startsWith("#") ? value : `#${value}`;
  if (!/^#[0-9a-fA-F]{6}$/.test(normalized)) return DEFAULT_PANDA_BROWSER_COLOR;
...
/**
 * Ensure the default "panda" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_PANDA_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_PANDA_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is a Panda browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for panda CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: PandaConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_PANDA_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "panda";
