...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".panda-profile-decorated");
}
 ...
 */
export function decoratePandaProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_PANDA_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_PANDA_BROWSER_COLOR).toUpperCase();
