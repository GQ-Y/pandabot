...
export function listSlackAccountIds(cfg: PandaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: PandaConfig): string {
...
function mergeSlackAccountConfig(cfg: PandaConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: PandaConfig): ResolvedSlackAccount[] {
