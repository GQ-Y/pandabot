...
metadata: {"panda":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Panda `sessions_spawn`
- **File I/O** == Panda `read`/`write`
- **Remote fetch** == Panda `web_fetch` (or `exec` with curl when POST is required)
