...
import type { PluginRuntime } from "pandabot/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.PANDABOT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-nostr-"));
  process.env.PANDABOT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.PANDABOT_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".pandabot");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.PANDABOT_STATE_DIR;
    else process.env.PANDABOT_STATE_DIR = previous;
