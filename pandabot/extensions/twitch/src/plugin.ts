/**
 * Twitch channel plugin for Panda.
 *
...
import { buildChannelConfigSchema } from "pandabot/plugin-sdk";
 ...
 * for Panda. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: PandaConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: PandaConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: PandaConfig): boolean => {
