...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: PANDABOT_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: PandaConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var PANDABOT_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => {
    ...
    return setTwitchAccessControl(cfg as PandaConfig, allowedRoles, true);
  },
  promptAllowFrom: async ({ cfg, prompter }) => {
    ...
    return setTwitchAccount(cfg as PandaConfig, {
      ...(account ?? undefined),
      allowFrom,
    });
  },
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.PANDABOT_TWITCH_ACCESS_TOKEN?.trim();
