...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => setZaloDmPolicy(cfg as PandaConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID
        : resolveDefaultZaloAccountId(cfg as PandaConfig);
...
export const zaloOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  dmPolicy,
  getStatus: async ({ cfg }) => {
    const configured = listZaloAccountIds(cfg as PandaConfig).some((accountId) =>
      Boolean(resolveZaloAccount({ cfg: cfg as PandaConfig, accountId }).token),
    );
    return {
      channel,
      configured,
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
    ...
    const defaultZaloAccountId = resolveDefaultZaloAccountId(cfg as PandaConfig);
