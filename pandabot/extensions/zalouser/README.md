...
Pandabot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
pandabot channels login --channel zalouser
...
```bash
pandabot message send --channel zalouser --target <threadId> --message "Hello from Pandabot!"
```
...
```bash
pandabot channels login --channel zalouser              # Login via QR
pandabot channels login --channel zalouser --account work
pandabot channels status --probe
pandabot channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
pandabot directory self --channel zalouser
pandabot directory peers list --channel zalouser --query "name"
pandabot directory groups list --channel zalouser --query "work"
pandabot directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
pandabot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
pandabot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `pandabot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
pandabot channels login --channel zalouser --account work
pandabot message send --channel zalouser --account work --target <id> --message "Hello"
