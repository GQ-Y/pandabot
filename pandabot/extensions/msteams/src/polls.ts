...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["pandaPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["panda", "pollId"]) ??
    readNestedString(value, ["panda", "poll", "id"]) ??
    readNestedString(value, ["data", "pandaPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "panda", "pollId"]);
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "panda poll vote",
          displayText: "Vote recorded",
          value: { pandaPollId: pollId },
        },
      },
    ],
  };
