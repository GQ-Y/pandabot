...
// NOTE: This extension is intended to be bundled with Panda.
// When running from source (tests/dev), Panda internals live under src/.
...
export function createLlmTaskTool(api: PandaPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via panda.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "panda-llm-task-"));
