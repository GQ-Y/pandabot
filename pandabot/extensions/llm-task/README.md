...
`panda.invoke --each`) without adding new Pandabot code per workflow.
...
This extension depends on Pandabot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Pandabot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.pandabot/extensions` as a standalone plugin directory.
