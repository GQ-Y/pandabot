...
export function listGoogleChatAccountIds(cfg: PandaConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: PandaConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: PandaConfig): ResolvedGoogleChatAccount[] {
