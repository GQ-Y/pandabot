...
function resolvePandaRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.PANDA_ROOT?.trim() || process.env.PANDABOT_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set PANDA_ROOT (or legacy PANDABOT_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolvePandaRoot();
