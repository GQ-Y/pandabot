...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `panda-e2e-${name}-`));
  const configDir = path.join(homeDir, ".pandabot");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          PANDA_CONFIG_PATH: configPath,
          PANDA_STATE_DIR: stateDir,
          PANDA_GATEWAY_TOKEN: "",
          PANDA_GATEWAY_PASSWORD: "",
          PANDA_SKIP_CHANNELS: "1",
          PANDA_SKIP_BROWSER_CONTROL_SERVER: "1",
          PANDA_SKIP_CANVAS_HOST: "1",
        },
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        PANDA_GATEWAY_TOKEN: inst.gatewayToken,
        PANDA_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          PANDA_GATEWAY_PORT: String(gwA.port),
          PANDA_GATEWAY_TOKEN: gwA.gatewayToken,
          PANDA_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          PANDA_GATEWAY_PORT: String(gwB.port),
          PANDA_GATEWAY_TOKEN: gwB.gatewayToken,
          PANDA_GATEWAY_PASSWORD: "",
        }),
