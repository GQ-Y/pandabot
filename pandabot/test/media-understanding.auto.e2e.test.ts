...
import type { PandaConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("panda-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("panda-bin-sherpa-");
      const modelDir = await makeTempDir("panda-sherpa-model-");
      ...
      const cfg: PandaConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("panda-bin-whispercpp-");
      const modelDir = await makeTempDir("panda-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("panda-bin-gemini-");
      ...
      const cfg: PandaConfig = { tools: { media: { image: {} } } };
