...
ZIP=${1:?"Usage: $0 Panda-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/panda/panda/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Panda-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/panda/panda/releases/download/v${VERSION}/"}
