...
BASE_IMAGE="${BASE_IMAGE:-panda-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-panda-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=panda.sandbox=1)
NOTE
