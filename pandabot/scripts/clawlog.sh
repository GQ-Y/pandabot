...
show_usage() {
    cat << EOF
clawlog - Panda Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Panda logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Panda logs flow through the macOS unified log (subsystem: bot.molt).
