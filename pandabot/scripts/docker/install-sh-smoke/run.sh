...
SMOKE_PREVIOUS_VERSION="${PANDABOT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${PANDABOT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="panda"
if [[ -z "${PANDABOT_INSTALL_PACKAGE:-}" && "$INSTALL_URL" == *"clawd.bot"* ]]; then
  DEFAULT_PACKAGE="pandabot"
fi
PACKAGE_NAME="${PANDABOT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
if [[ "$PACKAGE_NAME" == "panda" ]]; then
  ALT_PACKAGE_NAME="pandabot"
else
  ALT_PACKAGE_NAME="panda"
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (PANDABOT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${PANDABOT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "$PANDABOT_INSTALL_LATEST_OUT"
fi
