...
#
# Suggested cron: */30 * * * * /home/admin/panda/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Panda notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., panda-alerts)
...
STATE_FILE="$HOME/.pandabot/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Panda if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use panda
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Panda to $NOTIFY_PHONE..."
            panda send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Panda is down. Run: ssh l36 '~/panda/scripts/mobile-reauth.sh'" "urgent"
    exit 1
