...
const compiler = env.PANDA_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.PANDA_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.PANDA_RUNNER_LOG === "0") return;
  process.stderr.write(`[pandabot] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["panda.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
