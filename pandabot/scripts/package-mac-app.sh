...
# Build and bundle Panda into a minimal .app we can open.
# Outputs to dist/Pandabot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Pandabot.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Panda"
BUNDLE_ID="${BUNDLE_ID:-bot.panda.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/pandabot/pandabot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Panda/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :PandaBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :PandaGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Panda"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Panda"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Panda"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Panda" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Panda/Resources/Panda.icns" "$APP_ROOT/Contents/Resources/Panda.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Panda/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying PandaKit resources"
PANDAKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/PandaKit_PandaKit.bundle"
if [ -d "$PANDAKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/PandaKit_PandaKit.bundle"
  cp -R "$PANDAKIT_BUNDLE" "$APP_ROOT/Contents/Resources/PandaKit_PandaKit.bundle"
else
  echo "WARN: PandaKit resource bundle not found at $PANDAKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Panda"
killall -q Panda 2>/dev/null || true
