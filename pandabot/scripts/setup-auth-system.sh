#!/bin/bash
# Setup Panda Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'panda-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Panda message: Send warning via Panda itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/panda-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/panda-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/panda-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now panda-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status panda-auth-monitor.timer"
echo ""
echo "Quick commands:"
