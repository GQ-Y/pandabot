...
SERVER="${PANDABOT_SERVER:-l36}"

# Check auth status
termux-toast "Checking Panda auth..."

STATUS=$(ssh "$SERVER" '$HOME/panda/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/panda/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|PANDABOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/panda/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|PANDABOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Panda won't work until re-auth"
                ;;
        esac
        ;;
