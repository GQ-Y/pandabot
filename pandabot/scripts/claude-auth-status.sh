...
# Checks both Claude Code and Pandabot auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
PANDA_AUTH="$HOME/.panda/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    panda models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    pandabot_status=$(check_pandabot_auth 2>/dev/null || true)

    claude_expires=0
    pandabot_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        pandabot_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        pandabot_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$PANDA_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$pandabot_status" \
        --arg be "$pandabot_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            pandabot: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$pandabot_status" == EXPIRED* ]] || [[ "$pandabot_status" == MISSING* ]]; then
        echo "PANDA_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$pandabot_status" == EXPIRING* ]]; then
        echo "PANDA_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Panda Auth (~/.pandabot/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$PANDA_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$PANDA_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'panda doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active pandabot >/dev/null 2>&1; then
    echo -e "Panda service: ${GREEN}running${NC}"
else
    echo -e "Panda service: ${RED}NOT running${NC}"
fi
