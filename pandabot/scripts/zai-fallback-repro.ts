...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "panda-zai-fallback-"),
  );
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "panda.json");
  ...
  const sessionId =
    process.env.PANDABOT_ZAI_FALLBACK_SESSION_ID ?? randomUUID();

  const baseEnv: NodeJS.ProcessEnv = {
    ...process.env,
    PANDABOT_CONFIG_PATH: configPath,
