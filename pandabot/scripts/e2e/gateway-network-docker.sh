...
IMAGE_NAME="panda-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="panda-net-e2e-$$"
GW_NAME="panda-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "PANDABOT_GATEWAY_TOKEN=$TOKEN" \
	  -e "PANDABOT_SKIP_CHANNELS=1" \
	  -e "PANDABOT_SKIP_GMAIL_WATCHER=1" \
	  -e "PANDABOT_SKIP_CRON=1" \
	  -e "PANDABOT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
