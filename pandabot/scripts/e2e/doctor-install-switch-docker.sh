...
IMAGE_NAME="panda-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/panda-bin:$PATH"
  mkdir -p /tmp/panda-bin

  cat > /tmp/panda-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/panda-bin/systemctl

  cat > /tmp/panda-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/panda-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/panda"
  npm_entry="/tmp/npm-prefix/lib/node_modules/panda/panda.mjs"
  git_entry="/app/panda.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/panda-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/panda-gateway.service"
    if [ ! -f "$unit_path" ]; then
