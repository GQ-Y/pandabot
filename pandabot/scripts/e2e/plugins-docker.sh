...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/panda-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.pandabot/extensions"

  cat > "$HOME/.pandabot/extensions/demo-plugin.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/panda-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@panda/demo-plugin-tgz",
  "version": "0.0.1",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/panda-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@panda/demo-plugin-dir",
  "version": "0.0.1",
  "panda": { "extensions": ["./index.js"] }
}
JSON
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/panda-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@panda/demo-plugin-file",
  "version": "0.0.1",
  "panda": { "extensions": ["./index.js"] }
}
JSON
