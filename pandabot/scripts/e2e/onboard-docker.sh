...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/panda-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/panda-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/panda-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$HOME/panda"
    config_path="$HOME/.pandabot/panda.json"
    sessions_dir="$HOME/.pandabot/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    export HOME="$home_dir"
    mkdir -p "$HOME/.pandabot"
    # Seed a remote config to exercise reset path.
    cat > "$HOME/.pandabot/panda.json" <<'"'"'JSON'"'"'
{
