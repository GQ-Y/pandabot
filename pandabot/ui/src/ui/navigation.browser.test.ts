...
import { PandabotApp } from "./app";
import "../styles.css";

const originalConnect = PandabotApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("panda-app") as PandabotApp;
...
beforeEach(() => {
  PandabotApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__PANDA_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  PandabotApp.prototype.connect = originalConnect;
  window.__PANDA_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/panda/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/panda");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/panda/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__PANDA_CONTROL_UI_BASE_PATH__ = "/panda";
    const app = mountApp("/panda/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/panda");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/panda/sessions");
  });
