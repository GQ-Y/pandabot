...
import type { PandabotApp } from "./app";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as PandabotApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as PandabotApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as PandabotApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as PandabotApp);
    await loadDevices(host as unknown as PandabotApp);
    await loadConfig(host as unknown as PandabotApp);
    await loadExecApprovals(host as unknown as PandabotApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as PandabotApp);
    await loadConfig(host as unknown as PandabotApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as PandabotApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as PandabotApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__PANDA_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as PandabotApp, false),
    loadPresence(host as unknown as PandabotApp),
    loadSessions(host as unknown as PandabotApp),
    loadCronStatus(host as unknown as PandabotApp),
    loadDebug(host as unknown as PandabotApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as PandabotApp, true),
    loadConfigSchema(host as unknown as PandabotApp),
    loadConfig(host as unknown as PandabotApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as PandabotApp, false),
    loadCronStatus(host as unknown as PandabotApp),
    loadCronJobs(host as unknown as PandabotApp),
  ]);
}
