...
export function renderNode(params: {
  ...
  return html`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${label}</div>
      <div class="cfg-field__error">${t("config.unsupportedTypeRaw").replace("{{type}}", type)}</div>
    </div>
  `;
}
...
function renderSelect(params: {
  ...
  return html`
    <div class="cfg-field">
      ...
      >
        <option value=${unset}>${t("config.selectPlaceholder")}</option>
  ...
  return html`
    <div class="cfg-array">
      <div class="cfg-array__header">
        ...
        <span class="cfg-array__count">${t("config.itemsCount").replace("{{n}}", String(arr.length))}</span>
        <button
          type="button"
                  ...
                  ${anySchema
                    ? html`
                        <textarea
                          ...
                          placeholder=${t("config.jsonValuePlaceholder")}
                          rows="2"
                          .value=${fallback}
                          ?disabled=${disabled}
                          @change=${(e: Event) => {
