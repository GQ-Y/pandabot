  ...
  return html`
    <section class="card">
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field" style="min-width: 220px;">
          ...
          <input
            ...
            placeholder=${t("logs.searchLogsPlaceholder")}
          />
        </label>
        ...
        ${LEVELS.map(
          (level) => html`
            <label class="chip log-chip ${level}">
              ...
              />
              <span>${t(`logs.level${level.charAt(0).toUpperCase()}${level.slice(1)}` as "logs.levelTrace")}</span>
            </label>
          `,
        )}
      </div>
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${props.onScroll}>
        ${filtered.length === 0
          ? html`<div class="muted" style="padding: 12px;">${t("logs.noLogEntries")}</div>`
          : filtered.map(
              (entry) => html`
                <div class="log-row">
