 ...
 */
function truncatePubkey(pubkey: string | null | undefined, na: string): string {
  if (!pubkey) return na;
...
export function renderNostrCard(params: {
      ...
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            ...
            <span class="monospace" title="${publicKey ?? ""}">${truncatePubkey(publicKey, t("common.na"))}</span>
          </div>
          <div>
              ...
              <div class="status-list">
                ${picture
                  ? html`
                    ...
                    `
                  : nothing}
                ${name ? html`<div><span class="label">${t("channels.name")}</span><span>${name}</span></div>` : nothing}
                ${displayName
                ...
                ${about
                  ...
                  : nothing}
                ${nip05 ? html`<div><span class="label">${t("channels.nip05")}</span><span>${nip05}</span></div>` : nothing}
              </div>
            `
          : html`
  ...
  return html`
    <div class="card">
      <div class="card-title">${t("channels.nostrTitle")}</div>
      <div class="card-sub">${t("channels.nostrSubtitle")}</div>
      ${accountCountLabel}
            ...
            <div class="status-list" style="margin-top: 16px;">
              ...
              <div>
                ...
                <span class="monospace" title="${summaryPublicKey ?? ""}"
                  >${truncatePubkey(summaryPublicKey, t("common.na"))}</span
                >
              </div>
              <div>
