...
export function renderDebug(props: DebugProps) {
  ...
  const securityLabel =
    critical > 0
      ? `${critical} ${t("debug.securityCritical")}`
      : warn > 0
        ? `${warn} ${t("debug.securityWarnings")}`
        : t("debug.securityNoCritical");
  const securityInfoSuffix = info > 0 ? ` · ${info} ${t("debug.securityInfo")}` : "";
          ...
          <div>
            ...
            ${securitySummary
              ? html`<div class="callout ${securityTone}" style="margin-top: 8px;">
                  ${t("debug.securityAudit")} ${securityLabel}${securityInfoSuffix}. ${t("debug.runAuditCommand")}
                </div>`
              : nothing}
